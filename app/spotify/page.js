  ...
  async function handleFavorite(e, itemType, itemId, name, artistName, coverUrl, source, extraData) {
    ...
    try {
      ...
      if (ar.ok && acceso.ilimitado) {
        await handleSaveOffline(e, itemType, itemId, name, artistName, source, "", coverUrl);
        return;
      }
      await toggleFavorite(itemType, String(itemId), name, artistName, coverUrl, source, extraData);
      if (itemType === "album" && album?.tracks) {
        enqueueAlbum(name, album.tracks.map((t, i) => ({ key: String(t.id || `${itemId}-${i}`), name: t.name, artist: t.artist || artistName, cover: coverUrl, duration_ms: t.duration_ms || null, online_only: true })));
      } else {
        enqueueAlbum(name, [{ key: String(itemId), name, artist: artistName, cover: coverUrl, duration_ms: extraData?.duration_ms || null, online_only: true }]);
      }
        ...
        <div>
          ...
          {artist.tracks?.length > 0 && (
            <div style={{marginBottom:24}}>
              ...
              <div style={{background:"var(--panel)",border:"1px solid var(--border)",borderRadius:12,overflow:"hidden"}}>
                {artist.tracks.slice(0,80).map(t => <div key={t.id} onClick={()=>t.album_id && loadAlbum(t.album_id, t.source || "itunes", t.name)} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",borderBottom:"1px solid var(--border2)",cursor:t.album_id?"pointer":"default"}}>
                  ...
                  {t.preview_url && <button onClick={(e)=>{e.stopPropagation();window.dispatchEvent(new CustomEvent("aura-reproducir",{detail:{key:`preview-${t.id}`,title:t.name,artist:t.artist,cover_url:t.cover,preview_url:t.preview_url,duration_ms:t.duration_ms,source:"preview"}}))}} style={{background:(sonandoGlobal.playing&&String(sonandoGlobal.key)===`preview-${t.id}`)?"#22c55e":"rgba(124,92,252,.15)",border:"none",borderRadius:"50%",width:30,height:30,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><Ico d={(sonandoGlobal.playing&&String(sonandoGlobal.key)===`preview-${t.id}`)?<><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></>:<polygon points="5 3 19 12 5 21 5 3"/>} size={13} stroke="#fff" fill="#fff"/></button>}
                </div>)}
              </div>
            </div>
          )}
