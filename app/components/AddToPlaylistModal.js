...
export default function AddToPlaylistModal({ item, onClose }) {
  ...
  const IS = { padding: "10px 12px", borderRadius: 8, border: "1px solid #333", background: "#1a1a2e", color: "#fff", fontSize: "0.95em", outline: "none", width: "100%", boxSizing: "border-box" };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }} onClick={onClose}>
      <div style={{ background: "#0f0f1a", borderRadius: 16, padding: 25, maxWidth: 400, width: "100%", border: "1px solid #2a2a3e", position: "relative" }} onClick={e => e.stopPropagation()}>
        <h3 style={{ fontSize: "1.2em", marginBottom: 5 }}>➕ Agregar a playlist</h3>
        <p style={{ color: "#888", fontSize: "0.85em", marginBottom: 15 }}>
        ...
        {added ? (
          <div style={{ textAlign: "center", padding: 20 }}>
            ...
            <button onClick={onClose} style={{ marginTop: 10, padding: "8px 16px", borderRadius: 8, border: "none", background: "#7c5cfc", color: "#fff", cursor: "pointer" }}>OK</button>
          </div>
        ) : (
          <>
            {/* Create new playlist */}
            <button onClick={() => setShowNew(!showNew)} style={{ width: "100%", padding: "10px", borderRadius: 8, border: "1px dashed #7c5cfc", background: "transparent", color: "#7c5cfc", cursor: "pointer", marginBottom: 12, fontSize: "0.9em" }}>
              ✨ Crear nueva playlist
            </button>

            {showNew && (
              <div style={{ background: "#1a1a2e", borderRadius: 10, padding: 15, marginBottom: 12, border: "1px solid #2a2a3e" }}>
            ...
            {playlists.length > 0 && (
              <div>
                <div style={{ color: "#888", fontSize: "0.8em", marginBottom: 8 }}>Tus playlists:</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 200, overflowY: "auto" }}>
                  {playlists.map(pl => (
                    <button key={pl.id} onClick={() => handleAdd(pl.id, pl.name)} disabled={loading} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 8, border: "1px solid #2a2a3e", background: "#1a1a2e", color: "#ccc", cursor: "pointer", textAlign: "left" }}>
                      {pl.cover_url ? (
                        <img src={pl.cover_url} style={{ width: 36, height: 36, borderRadius: 6, objectFit: "cover" }} />
                      ) : (
                        <div style={{ width: 36, height: 36, borderRadius: 6, background: "#2a2a3e", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.1em" }}>🎵</div>
                      )}
                      <div>
                        <div style={{ fontWeight: 600, fontSize: "0.9em" }}>{pl.name}</div>
                        <div style={{ color: "#555", fontSize: "0.75em" }}>{pl.description || "Sin descripción"}</div>
                      </div>
                    </button>
                  ))}
                </div>
        ...
        <button onClick={onClose} style={{ position: "absolute", top: 12, right: 15, background: "none", border: "none", color: "#555", fontSize: "1.3em", cursor: "pointer" }}>✕</button>
      </div>
    </div>
  );
}
